...
- **`src/core/`** — Infrastructure:
  ...
  - `GeminiClient` — Gemini Interactions API client. `Ask(question, systemPrompt, prevInteractionId)` → `GeminiResponse{text, interactionId, error, statusCode, ok}`. Fallback model on 500/503 only (NOT on 429 — rate limit is key-wide). Response parse: `steps[]` where `type=="model_output"` → join `content[].text`.
...
- **Fallback:** 500/503'te model_fallback'a gecer. 429'da GECMEZ (rate limit tum key icin gecerli).
...
- No markdown rendering (plain text — Faz 2 will add imgui_markdown)
