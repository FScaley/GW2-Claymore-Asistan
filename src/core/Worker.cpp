...
const std::string Worker::SYSTEM_PROMPT =
    ...
    "story step, 'only after', 'requires', 'unavailable'), state that condition as the wiki words it next to that entry, whether "
    "or not the user asked. Never present a conditional option as unconditional. Structured fields "
    "(locations[], items, sub_collections) may be partial; the section text in content (Locations / "
    "Acquisition / Unlocking) is the authority - if the needed section is truncated, call gw2_wiki with "
    "section= to fetch it in full.\n"
    "\n"
